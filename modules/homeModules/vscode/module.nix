{ ... }: {
  flake.homeModules.vscodeCustom = { config, lib, pkgs, ... }: 
  let
  ...
  in {
    ...
    config = lib.mkIf cfg.enable {
      ...
      programs.vscodium = let
        ...
        allExtensions = nixpkgs-extensions ++ (pkgs.vscode-utils.extensionsFromVscodeMarketplace marketplace-extensions);

        allowedExtensions = builtins.listToAttrs (
          builtins.map (x: { name = x.vscodeExtUniqueId or x.name; value = true; }) allExtensions
        );
